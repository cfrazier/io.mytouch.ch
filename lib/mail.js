...
const createUser = (exports.createUser = (spec, cb) => {
	const { email, name } = spec;
	const message = ejs.renderFile(
		path.join(__dirname, "../templates/createUser.ejs"),
		{
			email,
			name,
			url: `${server.https ? "https" : "http"}://${server.hostname}/admin/`,
		},
		null,
		(error, html) => {
			mg.messages().send(
	...
	const message = ejs.renderFile(
		path.join(__dirname, "../templates/resetPassword.ejs"),
		{
			...
			url: `${server.https ? "https" : "http"}://${
				server.hostname
			}/admin/dashboard/account/?token=${token}`,
		},
		null,
		(error, html) => {
			mg.messages().send(
