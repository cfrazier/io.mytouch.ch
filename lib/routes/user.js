	...
	if (token) {
		...
		jwt.verify(token, config.jwt.secret, async (error, decoded) => {
			if (error) {
				...
				try {
					decoded.organizations = await Organization.find({
						users: decoded._id,
						active: true,
					})
						.select("_id")
...
const loginUser = (module.exports.login = (req, res, next) => {
	...
	// Check to see if the user is actually attempting a log in
	if (req.auth) return res.send(req.auth);
	if (email && password) {
		...
		User.findUser(email, password, (error, user) => {
			...
			if (user) {
				...
				const where = user.isAdmin ? { true: false } : { users: { _id: user._id } };
...
const createUser = (module.exports.create = (req, res, next) => {
	...
	if (
		...
		mongoose.model("Organization").findOne({ _id: organizationId }, (error, organization) => {
...
const updateUser = (module.exports.update = (req, res, next) => {
	...
	if (auth.isAdmin || auth._id == userId) {
		...
		if (updateUser.password) updateUser.password = bcrypt.hashSync(updateUser.password, 10);
		// Run the update
		mongoose
			.model("User")
