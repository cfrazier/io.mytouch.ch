	...
	if (
		...
		Organization.findById(organizationId, (error, organization) => {
			...
			venue.color =
				organization.colors[
					Math.floor(
						Math.random() * Math.floor(organization.colors.length)
					)
				];
...
const readVenue = (module.exports.read = (req, res, next) => {
	...
	Venue.findOne({ organizationId, _id: venueId, active: true })
		...
		.exec((error, venue) => {
			...
			getAvailable(venue._id, (error, available) => {
...
const createCheckIn = (module.exports.createCheckIn = (req, res, next) => {
	...
	Group.findById(groupId)
		...
		.exec((error, group) => {
			...
			Venue.findOne({ code: venueCode }, (error, venue) => {
				if (error)
					return res.send({
						error: { name: "createCheckIn", ...error },
					});
				...
				getAvailable(venue._id, async (error, available) => {
					if (error)
						return res.send({
							error: {
								name: "createCheckIn",
...
const getAvailable = (venueId, cb) => {
	Venue.findById(venueId, (error, venue) => {
