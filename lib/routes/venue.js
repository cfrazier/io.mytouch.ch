...
const generateCode = () => {
	return crypto
		...
		.map((char) =>
			isNaN(char)
				? char.toUpperCase()
				: String.fromCharCode(70 + parseInt(char)).toUpperCase()
		)
		.join("");
};

const getVenueCode = async () => {
	let clean = false;
	let code;
	while (!clean) {
		code = generateCode();
		const venue = await Venue.findOne({ code }).lean().exec();
		if (!venue) clean = true;
	}
	return code;
};
...
const listVenues = (module.exports.list = async (req, res, next) => {
	const {
		auth,
...
const updateVenueCode = (module.exports.updateCode = (req, res, next) => {
	const {
		auth,
		params: { organizationId, venueId },
	} = req;
...
const listCheckIns = (module.exports.listCheckIns = (req, res, next) => {
	const {
		auth,
		params: { organizationId, venueId },
		query: { format, timezone = "America/Los_Angeles", start, end },
	} = req;
	// Check for authorization first
	if (!auth)
		return res.send({
	...
	if (
		...
		const where = { "venue._id": ObjectId(venueId), enter: { $lte: Date.now() } };
		if (start) where.enter.$gte = new Date(start);
		if (end) where.enter.$lte = new Date(end);
				...
				switch (format) {
					case "csv":
						const parser = new Parser({
							...
							transforms: [
								(checkin) => {
									checkin.enter = checkin.enter
										.toLocaleString("en-US", {
											timeZone: timezone,
										})
										.replace(",", "");
									if (checkin.exit)
										checkin.exit = checkin.exit
											.toLocaleString("en-US", {
												timeZone: timezone,
											})
											.replace(",", "");
						...
						res.setHeader(
							"Content-Disposition",
							'attachment; filename="' + "checkins-" + Date.now() + '.csv"'
						);
